...
class KnowledgeBasesForAmazonBedrock:
    ...
    def create_vector_index(self, index_name: str):
        ...
        body_json = {
            # ┌───────────────────────────────────────────────────────────────┐
            # │ 1) INDEX‐LEVEL SETTINGS                                       │
            # └───────────────────────────────────────────────────────────────┘
            "settings": {
                "index.knn": "true",                  # Enable k-NN per shard
                "number_of_shards": 1,                # Single shard for simplicity
                "number_of_replicas": 0,              # No replicas (use ≥1 in prod)
                "knn.algo_param.ef_search": 512,      # Default search beam width (override per-query)
                #"knn.algo_param.index_thread_qty": 1, # Threads for graph construction
                #"knn.algo_param.search_thread_qty": 1,# Threads for k-NN search
                "refresh_interval": "30s"             # Bulk‐indexing tip: longer interval → faster ingest
            },
            ...
            "mappings": {
                # Dynamic Templates:
                # The dynamic template "strings_as_keyword" ensures that any string fields
                #  not explicitly defined end up as keywords. This helps when different files bring in additional fields.
                # "dynamic_templates": [
                #    {
                #        "strings_as_keyword": {
                ...
                # Union of Fixed Fields:
                # – We explicitly map common fields from reservations (ReservationID, GuestID, BookingTime, etc.),
                #  table data (e.g. Capacity, NearWindow, LastUsedTime) and menu details (Meal Name, Category, Price (USD), Description).
                # – If a field exists in more than one file (for example, TableID or Location), the same mapping is used.
                "properties": {
                    "ReservationID": { "type": "keyword" },
                    "GuestID": { "type": "keyword" },
                    "BookingTime": { "type": "date", "format": "yyyy-MM-dd HH:mm:ss" },
                    "Channel": { "type": "keyword" },
                    "TableID": { "type": "keyword" },
                    "Status": { "type": "keyword" },
                    "CheckInTime": { "type": "date", "format": "yyyy-MM-dd HH:mm:ss||strict_date_optional_time" },
                    "PartySize": { "type": "integer" },
                    "Location": { "type": "keyword" },
                    "Capacity": { "type": "integer" },
                    "NearWindow": { "type": "boolean" },
                    "LastUsedTime": { "type": "date", "format": "yyyy-MM-dd HH:mm:ss" },
                    "Meal Name": { "type": "keyword" },
                    "Category": { "type": "keyword" },
                    "Price (USD)": { "type": "float" },
                    "Description": { "type": "text" },

                    "vector": {
                        "type": "knn_vector",          # Declare this field as a k-NN vector
                        "dimension": 1024,              # Must match your embedding size exactly
                        "method": {
                            "name": "hnsw",            # Use the HNSW algorithm
                            "engine": "faiss",         # Backed by Faiss library
                            "space_type": "cosinesimil", # Distance metric: L2|cosinesimil|innerproduct

                            # ┌─────────────────────────────────────────────────┐
                            # │ 2a) HNSW BUILD PARAMETERS (require reindex)     │
                            # └─────────────────────────────────────────────────┘
                            "parameters": {
                                "m": 32,                # Number of links per node. ↑ → better recall & more RAM
                                "ef_construction": 200,# Beam width during index build. ↑ → better graph quality
                                
                                # ┌──────────────────────────────────────────┐
                                # │ 2b) DEFAULT SEARCH PARAMETER             │
                                # │     (can still override per-query)       │
                                # └──────────────────────────────────────────┘
                                "ef_search": 512       # Default search beam width if not in query
                            }
                        }
                    },

                    # ┌───────────────────────────────────────────────────────────┐
                    # │ 2c) ADDITIONAL FIELDS (metadata, text, etc.)              │
                    # └───────────────────────────────────────────────────────────┘
                    "text": {
                        "type": "text"             # Standard full-text field
                    },
                    "text-metadata": {
                        "type": "text",            # Arbitrary JSON text
                    }
                }
            }
        }
        ...
        try:
            ...
            print(
                f'Error while trying to create the index, with error {e}\nyou may unmark the delete above to '
